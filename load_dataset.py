...
def preprocess_data(train_data, valid_split):
    x_train_data = []
    y_train_data = []
    ...
    if valid_split == False:
        ...
        temp_dataloader_train = DataLoader(temp_dataset, shuffle=True, batch_size=16)
    else:
        ...
        temp_dataloader_train = DataLoader(temp_dataset_train, shuffle=True, batch_size=16)
        temp_dataloader_valid = DataLoader(temp_dataset_valid, shuffle=True, batch_size=16)
    return temp_dataloader_train, temp_dataloader_valid


def get_data_loaders(download = False, split='train'):
    traget_data = None
    valid_split = False
    ...
    traget_dataloader, test_dataloader = preprocess_data(traget_data, valid_split)
